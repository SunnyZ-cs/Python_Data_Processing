...
"""
Python Portfolio Project: Quilt Project
"""
...
def draw_eye(canvas, left, top, width, height, n):
    ...
    x_mid = left + 0.5 * (width - 1)
    y_mid = top + 0.5 * (height - 1)
    for i in range(n):
        x_add = (i / (n - 1)) * (width - 1)
        canvas.draw_line(x_mid, y_mid, left + x_add, top + height - 1)
...
def draw_quilt(width, height, n):
    ...
    for row in range(n):
        for col in range(n):
            left = row * patch_width
            top = col * patch_height
            choice = (row + col) % 4
            if choice == 0:
