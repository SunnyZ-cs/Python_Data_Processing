...
def draw_fixed(canvas):
    ...
    for year_index in range(len(YEARS)):
        ...
        canvas.create_text(x + TEXT_DX, height - SPACE, text=YEARS[year_index], anchor=tkinter.NW)
...
def compute_y(names, name, height, year_index):
    ...
    """
    rank = compute_rank(names, name, YEARS[year_index])
    rank_height = height - 2 * SPACE
    y_add = (rank / MAX_RANK) * rank_height
    y = SPACE + y_add
    return y
...
def draw_name(canvas, names, name, color):
    ...
    for year_index in range(len(YEARS)):
        ...
        rank = compute_rank(names, name, YEARS[year_index])
        canvas.create_text(x + TEXT_DX, y, text=name + ' ' + str(rank), anchor=tkinter.SW, fill=color)
        if year_index != 0:
            canvas.create_line(x, y, prev_x, prev_y, width=LINE_WIDTH, fill=color)
        prev_x = x
        prev_y = y
...
def handle_search(search_entry, search_out, names):
    """
    (Core Utility) Called for <return> key in lower search field.
