...
"""
Python Portfolio Project: Sand Project
"""
...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    if grid.get(x_to, y_to) != None:
        return False
    if (x_to == x_from - 1 and y_to == y_from + 1) or (x_to == x_from + 1 and y_to == y_from + 1):
        if grid.get(x_to, y_to - 1) != None:
            return False
    return True
...
def do_whole_grid(grid, brownian):
    ...
    (tests and code TBD)
    >>> grid = Grid.build([[None, 's', None], [None, 'r', None], [None, None, None]])
    >>> do_whole_grid(grid, 0)
    [[None, None, None], ['s', 'r', None], [None, None, None]]
    >>> grid = Grid.build([[None, None, None], ['s', 'r', None], [None, None, None]])
    >>> do_whole_grid(grid, 0)
    [[None, None, None], [None, 'r', None], ['s', None, None]]
    """
